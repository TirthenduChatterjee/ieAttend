...
export default function LeavePage() {
  ...
  const mine = useQuery({ queryKey: ['my-leaves'], queryFn: () => get('/leaves/me') })
  const balance = useQuery({ queryKey: ['leave-balance'], queryFn: () => get('/leaves/balance') })
  const all = useQuery({ queryKey: ['all-leaves'], queryFn: () => get('/leaves'), enabled: user?.role === 'Hr' })
  ...
  return <DashboardLayout active="Leave"><div className="grid gap-4 xl:grid-cols-[.8fr_1.2fr]">
    <Card className="p-5"><Heading>Apply for leave</Heading>
    ...
    <Card className="p-5"><Heading>{user?.role === 'Hr' ? 'Leave requests' : 'My leave requests'}</Heading><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[560px] text-left text-sm"><thead className="bg-slate-50 text-xs text-slate-500"><tr>{user?.role === 'Hr' && <th className="p-3">Employee</th>}<th className="p-3">Dates</th><th>Type</th><th>Days</th><th>Status</th>{user?.role === 'Hr' && <th>Action</th>}</tr></thead><tbody>
